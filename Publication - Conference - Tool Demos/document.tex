...
\durationTotal person hours
trying to obtain, download, use, and repackage \totalToolsTried
tools from ICSE and FSE's tool demonstration tracks between 
2011 and 2014.
...
%TODO more theory here; what's the base definitions you're working from? we're narrow, but from what to what? \
%Collberg calls in repeatibility, but Krishnamurthy disagrees
%See Tim's paper he sent

Our work is one specific foray into the topic of reproducibility 
in science, an area whose importance researchers and the public 
increasingly cite.
In a survey of 1,500 scientists across disciplines, the journal
Nature reported that 70\% of researchers have attempted
and failed to replicate others' experiments~\cite{baker20161}.
Reproducibility in computer science is no less important than in
...
Several other computer science subdisciplines have studied
reproducibility.
Kova{\v{c}}evi{\'c} evaluated 15 image processing papers to evaluate
how well the papers described their algorithms and whether 
implementations were available; she found that descriptions were
...
we used and repackaged that code when possible.

Callberg and Proebsting's recent repeatability work~\cite{collberg2016repeatability,proebsting2015repeatability},
published after we had completed our study, is most similar to ours. 
In it, the authors examined the systems desmcribed in 
601 papers from 13 systems conferences from 2011 to 2013
(including, relevant to this paper, OOPSLA and PLDI 2012).
Of those papers:
...
\item in 146 cases, authors could not provide the code; and
...
\noindent
Apart from studying software engineering tool demonstrations rather than
technical papers from systems venues,
...
%TODO is this Tim's paper on Repeatability in AI-SE: ``depends on who does it''?
...
variety and a larger number of software engineering tools. 

\subsection{Artifact Evaluation Committees}

Researchers in programming languages, and to a lesser extent
software engineering, have recently put their desire to 
improve the state of reproducibility in research by forming
...
submit artifacts to AECs or to archive those artifacts
after the conference.

Several past instantiations of AECs have reported data about 
their outcomes; we summarize those that we are aware of
in Table~\ref{tbl:aecs},
where we list the name of the conference,
the number of research papers that were accepted,
artifacts submitted,
and artifacts accepted.
Overall, it appears that about half of paper authors choose
to submit artifacts; and acceptance rates for those
artifacts vary substantially.

\begin{table}[t]
  \begin{center}
    \begin{tabular}{rrrr}
    Conference & Papers & Artifacts & Accepted\\ 
    \hline
    FSE 2011\tablefootnote{\url{http://cs.brown.edu/~sk/Memos/Conference-Artifact-Evaluation/esec-fse-2011.html}} & 34 & 14 & 7 \\
    OOPSLA 2013\tablefootnote{\url{http://evaluate.inf.usi.ch/artifacts/aea/statistics}} & 51 & 21 & 18 \\
    PLDI 2014\tablefootnote{\url{http://pldi14-aec.cs.brown.edu/}} & 51 & 20 & 12 \\
    POPL 2015\tablefootnote{\url{http://popl15-aec.cs.umass.edu/popl15_report/}} & 52 & 29 & 27 \\
    \end{tabular}
  \end{center}
  \caption{Data from artifact evaluation 
  committees.}\label{tbl:aecs}
\end{table}
...
we report on tools that, generally, we package outselves as a third-party.
...
fundamental reasons tools cannot be redistributed. 
...
\caption{An email template used by students for obtaining information about tools from authors.}\label{fig:email}
...
The reason is that we do not wish to embarass any of the authors.
Accordingly, we have taken steps to make sure that data we have made 
public with this paper does not embarass authors either.
...
were fixed by the tools' authors.
...
\subsubsection{Author Compliance}
...
After contacting the authors, teams reported that authors 
...
%TODO is this a correct use of term ``virutalizing''?
...
\licenseLGPL has a LGPL 2.0 license,
\licenseMPL has a Microsoft Public License.
Authors added open source licenses to \contactOSSed tools
...
%TODO One author said tool just doesn't exist anymore.
%TODO Another author had to dig tool out of long term archive.
%TODO Several authors (what percent?) has tools hosted on Google code, even though it was dying. Did we save 'em?
...
What are the implications for authors? For conferences?
...
Consider prolem of Topaz (?), system that
